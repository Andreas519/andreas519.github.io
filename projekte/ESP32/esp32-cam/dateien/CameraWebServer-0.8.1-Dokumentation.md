# ESP32-CAM CameraWebServer 0.8.1
...
Die aktuelle Firmware-Version ist **0.8.1**.
...
| `/ble-mode` | per POST-Request neu in den BLE-Modus starten |
| `/status` | aktuelle Kameraeinstellungen als JSON |
| `/control` | Kameraeinstellungen ändern |

## Wechsel in den BLE-Modus per HTTP

Im Webserver-Modus kann das Modul ohne Zugriff auf den Taster in den
BLE-Konfigurationsmodus versetzt werden:

```powershell
Invoke-RestMethod -Method Post -Uri http://192.168.x.x/ble-mode
```

Die Antwort bestätigt den bevorstehenden Neustart:

```json
{"status":"restarting","mode":"ble"}
```

Das Modul merkt den einmaligen BLE-Start im NVS vor und startet anschließend
neu. Danach ist der Webserver nicht mehr erreichbar und das Modul meldet sich
über BLE. Der Endpunkt besitzt keine Anmeldung und darf nur in einem
...
Der praktische Test der in Version 0.8.1 unveränderten Fotoübertragung erfolgte
bereits mit Version 0.8.0 und übertrug erfolgreich ein JPEG mit
...
- `POST /ble-mode` als JSON bestätigt
- automatischen Neustart vom Webserver- in den BLE-Modus bestätigt

## Download

Der aktuelle Firmware-Quellcode steht als
`dateien/CameraWebServer-v0.8.1.zip` bereit. Vor der Veröffentlichung wurde
