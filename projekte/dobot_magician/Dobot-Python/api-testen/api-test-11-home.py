...
Startet eine HOME-Fahrt über die Queue.
...
try:
    ...
    #    [200, 100.0, 80.0, 0.0] waren die Orginalwerte
    home_params_index = dType.SetHOMEParams(
        api,
        200.0,
        0.0,
        -20.0,
    ...
    if aktive_alarme:
        raise RuntimeError(
            "HOME wird nicht gestartet, weil Alarme aktiv sind: "
            + ", ".join(
                f"0x{nummer:02X}"
    ...
    bestaetigung = input(
        "Arbeitsraum frei? HOME-Fahrt starten? (j/n): "
    ).strip().lower()

    if bestaetigung != "j":
        print("HOME-Fahrt wurde nicht ausgeführt.")
    else:
        ...
        print("HOME-Queue-Index:", ziel_index)
        print("HOME-Fahrt läuft ...")
        ...
        print(
            f"Position: X={pose[0]:.2f}, Y={pose[1]:.2f}, "
